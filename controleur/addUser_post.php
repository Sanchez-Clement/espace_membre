...
// comparaison des deux passwords saisis

if ($passw != $password2) {
    $_SESSION["error"]["password_register"] = "Mot de pass différent";
    $error = 1;
} else {
    $_SESSION["error"]["password_register"] ="";
    $passw = password_hash($passw, PASSWORD_BCRYPT);
};


// verification si mail conforme

if (preg_match("#^[a-z0-9._-]+@[a-z0-9._-]{2,}\.[a-z]{2,4}$#", $mail)) {
    $_SESSION["error"]["mail_register"] ="";
} else {
    $_SESSION["error"]["mail_register"] = "format non conforme !";
    $error = 1;
} ;


// verification de l'extension et du poids de l'image
...
if (!in_array($_FILES["avatar"]["type"], $extension)) {
    ...
    $_SESSION["error"]["avatar_register"] = "";
};


// Si erreurs redirection vers formaulaire sinon ajout en base de donnés

if (isset($error)) {
    header("Location: ../index.php?section=adduser");
} else {
